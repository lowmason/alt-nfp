...
**Track B (out of scope for THIS plan — built separately in `plans/14`).** The product that competes with **consensus** is **Total NFP = private nowcast + a government forecast**. That government forecast — the `00 − 05` **wedge** — has since been **built** (`specs/government_wedge.md` / `plans/14`: `nfp_model/wedge.py`, `assemble_total`, `score_total`, `cmd_total`), so it is no longer the undesigned critical path it was when this plan was written. Consensus is a **Total** object; it has no meaning against the private nowcast alone, so it stays **removed from the private (Track A) scoreboard and from the private MZ** — this plan builds **Track A only** (the private nowcast vs naive floors); the Total/consensus contest lives in `plans/14`. See `specs/model_improvements.md` §2/§9.

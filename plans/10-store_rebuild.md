...
| **T7** Re-baseline goldens | ✅ **unblocked 2026-06-17 ([plans/11](11-model_data_dropin.md))** | drop-in delivered: `build_model_data(rebuilt)` non-degenerate (`qcew_obs=71`, `g_ces_sa=77`; was 0/0), 7 `real_store` gates green, `00` band calibrated. Re-baseline proceeds. See T7. |
| **T8** Promotion | ⏳ depends T7 + maintainer GO | T7 unblocked; the unchanged model now consumes the rebuilt store. Awaiting the goldens re-baseline (T7) + cutover GO. |
...
## T7 — Re-baseline A1/A2 goldens `[depends: T6 passing]` — ✅ UNBLOCKED 2026-06-17 (plans/11 drop-in delivered + verified)

...
| **T8** Promotion | ⏳ **T7 done; awaiting maintainer GO** | Re-baseline complete; store + scratch goldens ready to promote together (copy `store-rebuild`→`store`, `a1-rebuild`/`a2-rebuild`→`golden/a1`/`a2`, merge `goldens-rebaseline`, snapshot prior canonical first). Awaiting cutover GO. |

...
**⛔ Open DEFECT for the maintainer — gates T7 (must be ruled on before re-baselining goldens).** The store's **Q1 all-sizes** headline (`size_class_code='0'`) is **light vs the published, un-suppressed BLS area total** — i.e. the rebuild is *wrong*, not just "different from a wrong oracle." Mechanism: the §7 compose anti-joins away the (correct) area-levels null-size row and substitutes the size-cross-product `total/'0'` = **sum of the available native size buckets**; agglvl 13–16 is **not** suppressed by BLS, so the published all-sizes total is real and known, and the bucket-sum undercounts it whenever buckets are missing (suppression **or** unpublished frontier tables). **Quantified across all store years (2017–2025), Q1, vs the live area endpoint (`under = published − store`):**

-   **Settled years 2017–2024 — suppression undercount:** headline `total/05` max **~3.9k jobs (0.003%)** — negligible at the top — but suppression-heavy sectors bite relatively: sector `11` up to **8.5%**, sector `32` (nondurable mfg) up to ~1% (~45k jobs in the worst year), supersector `10` ~0.66%. Small but real, and worst exactly where a supersector/sector "why" narrative looks.
-   **Frontier 2025-Q1 — CATASTROPHIC (the partial-size-table case):** the size tables were largely unpublished at build time, so the partial bucket-sum replaced the full area total — `total/05` is **23.4M jobs / ~18% too low**; supersectors `20`/`30`/`10`/`50` are **81–93% under**; `70` ~6.6%, `80` ~30%. The store's *most recent* Q1 — exactly what the nowcast reads — is badly broken. (No row is ever an *over*count; the substitution only ever loses employment.)

The §7 substitution thus turns a *benign* frontier condition (size detail not yet published) into a *malignant* one (headline 18% wrong). **Why it gates T7:** T7 re-baselines A1/A2 goldens *from this store*, which would bless this undercount as truth. **Identified fix (compose-level, not a one-line source swap):** have the Q1 all-sizes headline carry the **area-levels** total (keep the area null-size row; let the size *buckets* be sub-detail that legitimately need not sum to the total when cells are suppressed/unpublished) — changes §7/§8 + the T4 "Q1 emits `total/'0'` only" invariant, and requires a maintainer re-run of `build-rebuild`. Tracked: `specs/store_rebuild.md` §7/§10, `specs/size_classes.md`, spawned task. *(The 2025-Q1 frontier lag was already noted under T5 as "fills on the next rebuild"; this finding shows it also corrupts the headline until §7 is fixed.)*

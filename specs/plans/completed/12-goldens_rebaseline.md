...
**Scratch-prefix integrity (for T8 whole-prefix promotion).** The uploader writes by filename and never deletes, so a stale fixture from a prior run could linger invisibly (the manifest-driven tests would not see it, but a whole-prefix copy at T8 would promote it). Verified clean: `s3://alt-nfp/golden/a1-rebuild` = exactly **11** objects (9 panels + provider + manifest), `…/a2-rebuild` = exactly **28** (9×{npz,levels,panel} + manifest); no orphan `panel_asof_2026-01-12.parquet`. A whole-prefix copy at T8 is safe.
